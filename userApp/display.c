...
int showRules(struct FWRule *rules, int len) {
	...
	//printf("rule num: %d\n", len);
	printLine(111);
	printf("| %-*s | %-16s | %-16s | %-11s | %-11s | %-6s | %-4s | %-3s |\n", MAXRuleNameLen,
	 "name", "src ip", "dest ip", "src port", "dest port", "protocol", "action", "log");
	printLine(111);
	for(i = 0; i < len; i++) {
		showOneRule(rules[i]);
	}
...
int showOneLog(struct logItem log) {
	...
	sprintf(tm, "%4d-%02d-%02d %02d:%02d:%02d",1900 + timeinfo->tm_year, 1 + timeinfo->tm_mon, timeinfo->tm_mday, timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec);
...
void processKernelAnswer(struct kernelAnswer rsp) {
	...
	case RSP_MSG:
		...
		break;
	case RSP_IPRules:
		showRules((struct FWRule*)rsp.body, rsp.header->arrayLen);
		break;
	case RSP_NATRules:
		showNATRules((struct NATRecord*)rsp.body, rsp.header->arrayLen);
		break;
	case RSP_IPLogs:
		showLogs((struct logItem*)rsp.body, rsp.header->arrayLen);
		break;
	case RSP_ConnLogs:
		showConns((struct ConnLog*)rsp.body, rsp.header->arrayLen);
		break;
	}
	if(rsp.header->type != RSP_Only_Head && rsp.body != NULL) {
		free(rsp.data);
	}
}