...
// 比较连接标识符
int connectioncmp(unsigned int a[3], unsigned int b[3]) 
{
	int i=0;
	for(i=0;i<3;i++) 
		if(a[i] != b[i])
...
struct connectionNode *searchNode(struct rb_root *root, unsigned int key[3])
{
	int result;
	struct rb_node *node;
	read_lock(&connLock);
	node = root->rb_node;
	while (node) {
		struct connectionNode *data = rb_entry(node, struct connectionNode, node);
		result = connectioncmp(key, data->key);
		if (result < 0)
			node = node->rb_left;
		else if (result > 0)
			node = node->rb_right;
...
struct connectionNode *insertNode(struct rb_root *root, struct connectionNode *data)
{
	...
	while (*new) {
		struct connectionNode *this = rb_entry(*new, struct connectionNode, node);
		int result = connectioncmp(data->key, this->key);
		parent = *new;
		if (result < 0)
			new = &((*new)->rb_left);
		else if (result > 0)
...
{// 初始化
	struct connectionNode cn,*node=NULL;
	node = (struct connectionNode *)kzalloc(sizeof(cn), GFP_ATOMIC);
	if(node == NULL) {
...
unsigned short getNewNATPort(struct NATRule rule)
{
	...
	for(port = rule.nowPort + 1; port != rule.nowPort; port++)
	{
		...
		{
			now = rb_entry(node, struct connectionNode, node);
...
int eraseConnRelated(struct FWRule rule)
{
	...
	struct connectionNode *needDel = NULL;
	...
	{ // 有更改时，持续遍历，防止漏下节点
		...
		{
			needDel = rb_entry(node, struct connectionNode, node);
			sport = (unsigned short)(needDel->key[2] >> 16);
			dport = (unsigned short)(needDel->key[2] & 0xFFFFu);
			if(getMatched(&rule, needDel->key[0], needDel->key[1], sport, dport, needDel->protocol))
			{ // 相关规则
				hasChange = 1;
				break;
			}
